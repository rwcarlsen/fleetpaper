...
One of the major motivations for the development of Cyclus was creating a
simulation environment that is more expressive and less limited than the more
traditional system dynamics based simulators. One of the defining differences
between these two modeling paradigms involves the discretization of
facilities. System dynamics simulators such as VISION simulate facility
"fleets". In system dynamics fleet-based modeling:
...
\begin{itemize}
    ...
    \item resource flows are continuous (limited by time step discretization)
...
with the intent of modeling facilities individually with discrete flows. The
difference between these two modeling styles in terms of their affects on
output of fuel cycle simulators in general is currently unknown. This
analysis is designed to provide both quantitative and qualitative insight
about how fleet-based continuous flow models differ from individual facility
...
investigated together with fleet vs facility modeling.  Questions of
interest include:
...
reactor modeling and time step duration.

\section{Fleet Modeling Overview}

There are various low level phenomena that have potential to affect the larger
outcome of simulations with respect to facility modeling decisions.  One
potential difference caused by fleet-based modeling involves fuel sharing.
Power production by a group of individual reactors for a fixed amount of fuel
shortage may vary depending on how the existing fuel is distributed.  This is
illustrated in Figure \ref{fig:fuel-sharing}.  Being short 3 batches might
mean that one reactor has no batches and isn't operating or that 3 reactors
are missing a single batch.  This will be referred to as the \emph{fuel
sharing effect}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.70\textwidth]{exp2/fuel-sharing.pdf}
...
Another difference between the fleet and individual modeling is the
...
modeling, power goes offline in for full time steps.  For example, if a
3-batch fleet based reactor is short $\frac{1}{2}$ batch of fuel with a 3
month time step, then it will produce $\frac{5}{6} \cdot 3 \text{mo} \cdot C $ of
energy.  In contrast, a monthly time step might allow the same reactor to
produce $[\frac{5}{6} \cdot 1 \text{mo} + 2 \text{mo}] \cdot C $ of energy if there was
actually a fuel shortage for only 1 month.
...
then all reactors would be offline refueling at the same time.  This is
referred to as the \emph{cycle staggering effect} and does not occur in
fleet-based facility models.  An example of poor cycle staggering is shown in
Figure \ref{fig:sync-cycle}.  This effect is closely related to the
\emph{inventory drawdown effect} described next.

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{exp2/sync-cycle.eps}
...
Another consequence of synchronized reactor cycles is fuel fabrication
capacity idling for an entire cycle (e.g. 18 months), and then receiving
requests for new batches for every reactor all at once.  The reactors draw out
inventory from the fuel fabrication facility together in a large quantity of
orders. In order to avoid running short of inventory for making fresh fuel,
the fabrication facility must keep at enough material on-hand to satisfy all
the reactors at once.  If the reactor cycles were staggered, then a much lower
on-hand inventory would be sufficient to prevent fuel shortages. This is
referred to as the \emph{inventory drawdown effect}.  Increasing time step
duration can also cause similar effects as it reduces the time granularity
over which refueling can occur, resulting in larger impulse drawdown on
inventories. Another component of this effect is that incoming and outgoing
material flows occur "simultaneously" on a given time step. Facilities do not
...
for making offers. In general longer time steps and individual reactor
modeling create a need for larger floating inventory buffers in order to avoid
...
A fleet reactor facility was created for Cyclus.  It is possible to create
fleet models for Cyclus that live anywhere along the spectrum from individual,
discrete facilities to continuous flow fleet facilities.  The particular model
created here falls more toward the aggregate, continuous flow side of the
spectrum.  
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{exp2/flow.eps}
...
reactor behavior are shown in Table \ref{tab:exp2-invar}.  Table
\ref{tab:exp2-reactors} shows the computed/selected configuration for both
...
Figure \ref{fig:exp2-power} shows an overall view of the generated
power over time for each of the four cases.  Figure
\ref{fig:exp2-power-rel} normalizes the Figure
\ref{fig:exp2-power} curves to the expected exponential power curve
...
\begin{figure}[h]
    ...
    \label{fig:exp2-power-rel}
\end{figure}

\subsection{Fuel Shortage}

Figure \ref{fig:exp2-unfueled} shows the fuel shortage more explicitly
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{exp2/unfueled.eps}
    ...
    \label{fig:exp2-unfueled}
\end{figure}

Quantifying imperfect fuel sharing is a bit difficult, but Figure
\ref{fig:exp2-badshare} gives one way to approximate the \emph{fuel
...
Figure \ref{fig:exp2-unfueled} suggesting that most of the discrepency
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{exp2/badshare.eps}
