...
domain.  Four scenarios were compared using combinations of either fleet-based
or individual reactors with either monthly or quarterly (3-month) time steps.
When under supply-constraint pressure, the four cases exhibit noticably
different behavior.  Otherwise, they generally behaved very similarly.
Fleet-based modeling is more efficient in supply-constrained environments at
the expense of losing insight on issues such as realistic sub-optimal fuel
distribution and challenges in reactor refueling cycle staggering.
Finer-grained time steps also enable more efficient material use in
supply-constrained environments.  Large simulations with fleet-based reactors
run much more quickly than their individual reactor counterparts.
...
minimum bound for outage durations. The effects of the time step duration will
also be investigated together with fleet vs facility modeling.
...
flexible and scriptable usage and input file format.  Cyclus holds its diverse
