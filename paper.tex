...
suboptimal fuel distribution and challenges in reactor refueling cycle
...
outcome of simulations with respect to facility and time step modeling
...
durations (i.e. cases 2 and 4).  Because the fleet reactors have perfect fuel
sharing (described below) and no outage noise, differences observed will
...
wait until enough fuel can be pumped through fabrication.  Avoiding such
...
this effect is that a single material object can only move between a single
pair of facilities per time step. Larger time steps result in longer minimum
...
different time step durations (e.g. cases 2 and 4).
...
amount of work being done in the simulation is the same; in the monthly time
step cases most facilities (i.e. all reactors) are only perform actions every
...
reactors in cases 1 and 3 actually go offline and back online for refueling
...
shortage lasts until about year 140. The power generated in Case 4 is slightly
lower than in case 2 during the fuel shortage years. This is primarily due to
the \emph{quantized shutdown effect}.  The longer 3-month time step in case 4
...
\begin{figure*}[!h]
    ...
    \caption[Normalized power]{
        ...
        excessive divergence above and below $1.0$ in cases 1 and 3 where
...
step of cases 3 and 4 also make the outage significantly more severe. Cases 1
...
$\text{GWe} \cdot \text{months}$ for cases 1 and 3 respectively.  These
...
\begin{figure*}[!h]
    ...
    \caption[Separated Pu inventory and flow]{
        ...
        cases 1 and 3 are omitted because they are noisy and obscure other
        ...
        with larger time steps that can be seen in case 4 than in case 2.
    }
    \label{fig:puinv}
\end{figure*}
...
fleet and discreet modeling done here is itself multi-dimensional. If fuel
sharing is of interest, but cycle staggering is not, individual reactors can
be used with the power capacity lowered and the outage reduced to zero
duration making the capacity factor included in the reactor's power capacity.
...
following shortage-induced outages should not follow the natural pacing of
fuel availability, otherwise refueling cycles become unsatisfactorily
synchronized.

Increasing the time steps increases facilities' standing inventory requirement
for avoiding supply constraints.  Facilities can provide no more inventory
than they can hold on a single time step.  So increasing time step duration
for a facility with unconstrained supply generally means higher standing
inventories are required in order to achieve equal throughput. Facilities that
have unconstrained supply and very large inventory buffers, are not affected
meaningfully by the time step. Even when facilities do not have individual
...
reactors).  The \emph{inventory drawdown effect} can impact the aggregate
